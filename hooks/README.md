...
| `block-destructive-bash.sh` | `PreToolUse` (Bash) | Blocks bash commands that bypass the trash and can wipe work: a recursive-and-forced `rm`, a `find ... -delete`, and a forced `git clean`. Use `trash`, or stage/stash instead. Writes a line to `guard.log` on every block. It stops accidents, not deliberate evasion: variable indirection (`X=rm; $X -rf`), `eval`, piping into a shell, and calling out to another language's delete function are all invisible to a regex over unexpanded text, and are not attempted. |
