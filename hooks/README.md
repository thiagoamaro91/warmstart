...
tool-call time (the four PreToolUse guards). All are plain bash. The only runtime
...
- `PreToolUse` matcher `Write|Edit` -> `block-em-dash.sh`, then `guard-memory-size.sh`, then `guard-context-index-size.sh`
...
runs the regression suite for the four PreToolUse guards: a syntax check plus behavioral
